            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr class="text-dark">
                        ...
                        <td>
                            @php $status = $reminder->status; @endphp
                            <span class="badge 
    ...
    <nav>
        <ul class="pagination pagination-sm pagination-gutter justify-content-center">
            ...
            <li class="page-item {{ $activeReminders->onFirstPage() ? 'disabled' : '' }}">
                <a class="page-link"
                   href="{{ $activeReminders->previousPageUrl() ? $activeReminders->previousPageUrl() . '&' . http_build_query(request()->except('active_page')) : '#' }}">
            ...
            @for ($i = 1; $i <= $activeReminders->lastPage(); $i++)
                <li class="page-item {{ $i == $activeReminders->currentPage() ? 'active' : '' }}">
                    <a class="page-link"
                       href="{{ $activeReminders->url($i) . '&' . http_build_query(request()->except('active_page')) }}">
                        {{ $i }}
                    </a>
                </li>
            @endfor
            ...
            <li class="page-item {{ !$activeReminders->hasMorePages() ? 'disabled' : '' }}">
                <a class="page-link"
                   href="{{ $activeReminders->nextPageUrl() ? $activeReminders->nextPageUrl() . '&' . http_build_query(request()->except('active_page')) : '#' }}">
                ...
                <tbody>
                    ...
                    <tr class="text-dark">
                        ...
                        <td>
                            @php $status = $reminder->status; @endphp
                            <span class="badge 
                                {{ $status === 'missed' ? 'bg-danger' : 'bg-success' }}">
                                {{ ucfirst($status) }}
                            </span>
                        </td>
                    </tr>
    ...
    <nav>
        <ul class="pagination pagination-sm pagination-gutter justify-content-center">
            ...
            <li class="page-item {{ $completedReminders->onFirstPage() ? 'disabled' : '' }}">
                <a class="page-link"
                   href="{{ $completedReminders->previousPageUrl() ? $completedReminders->previousPageUrl() . '&' . http_build_query(request()->except('completed_page')) : '#' }}">
                    <i class="icon-arrow-left"></i>
                </a>
            </li>
            ...
            @for ($i = 1; $i <= $completedReminders->lastPage(); $i++)
                <li class="page-item {{ $i == $completedReminders->currentPage() ? 'active' : '' }}">
                    <a class="page-link"
                       href="{{ $completedReminders->url($i) . '&' . http_build_query(request()->except('completed_page')) }}">
                        {{ $i }}
                    </a>
                </li>
            @endfor
            ...
            <li class="page-item {{ !$completedReminders->hasMorePages() ? 'disabled' : '' }}">
                <a class="page-link"
                   href="{{ $completedReminders->nextPageUrl() ? $completedReminders->nextPageUrl() . '&' . http_build_query(request()->except('completed_page')) : '#' }}">
                    <i class="icon-arrow-right"></i>
                </a>
            </li>
        </ul>
